...
public class CustomerService {
    ...
    public List<OrderItem> getOrderItemsByCustomerAndOrder(int customerId, int orderId) {
        ...
        return customer.getOrders().stream()
                .filter(order -> order.getOrderId().equals(orderId))  // check orderId
                .findFirst()
                .map(UserOrder::getItems)  // fetch items
                .orElseThrow(() -> new RuntimeException("Order not found for this customer"));
    }
...
public CustomerOrder getOrderDetails(Integer orderId) {
    return customerRepository.findOrderWithItems(orderId)
            .orElseThrow(() -> new RuntimeException("Order not found"));
}

// ✅ Get all orders placed by a buyer
public List<CustomerOrder> getOrdersByUser(Integer userId) {
    return customerRepository.findOrdersByUserId(userId);
}
}
